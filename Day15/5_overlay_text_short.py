...
intro_duration = 5
intro_music = bacground_audio_clip.subclip(0, intro_duration)
intro_text = TextClip("Hello World!", color='white', size=video_clip.size)
...
watermark_size = 60
watermark_text = TextClip("E&VF", fontsize=watermark_size, color='white', align='West', size=(w,watermark_size))
watermark_text = watermark_text.set_fps(fps)
watermark_text = watermark_text.set_duration(video_clip.duration)
watermark_text = watermark_text.margin(left = 50, right=10, bottom=50, opacity = 0)
watermark_text = watermark_text.set_position("bottom")

# Dolezite je poradie v akom compositujeme videa na seba
# watermark -> text ovberlay by mal byť posledny
# Zíkladné video musí byť na spodu stock..... musí sa s ním začínať
...
oberlay_clip = CompositeVideoClip([video_clip, watermark_text], size=video_clip.size)
oberlay_clip = oberlay_clip.set_duration(video_clip.duration)
oberlay_clip = oberlay_clip.set_fps(fps)
oberlay_clip = oberlay_clip.set_audio(None)

og_audio = AudioFileClip(og_audio_path)
oberlay_clip = oberlay_clip.set_audio(og_audio)

final_clip = concatenate_videoclips([intro_text, oberlay_clip])
